...
Parse.Cloud.define('initCollections', async (request) => {
  // Utiliser la Master Key pour cette opération
  ...
  for (const collection of collections) {
    try {
      // Vérifier si la collection existe déjà en essayant de créer un objet
      const TestObject = Parse.Object.extend(collection);
      const testObj = new TestObject();
      
      // Essayer de sauvegarder un objet test pour vérifier si la collection existe
      // Nous allons utiliser une approche différente - essayer de faire une requête
      const query = new Parse.Query(collection);
      query.limit(0); // Ne retourner aucun résultat, juste vérifier si la collection existe
      
      try {
        await query.find({ useMasterKey: true });
        console.log(`Collection ${collection} existe déjà.`);
        results.push({ collection: collection, status: 'exists', message: 'Collection existe déjà' });
        continue; // Passer à la collection suivante
      } catch (queryError) {
        // Si la requête échoue, la collection n'existe probablement pas
        console.log(`Collection ${collection} n'existe pas, création...`);
      }
      
      // La collection n'existe pas, la créer
      const schema = new Parse.Schema(collection);
      
      // Définir les champs pour la collection listes_relances
      if (collection === 'listes_relances') {
        schema.addField('nom', { type: 'String', required: true });
        schema.addField('is_auto', { type: 'Boolean', defaultValue: false });
      ...
      results.push({ collection: collection, status: 'created', message: 'Collection créée avec succès' });
      
    } catch (err) {
      console.error(`Erreur lors du traitement de la collection ${collection}:`, err);
      results.push({ collection: collection, status: 'error', message: err.message });
    }
  }
  ...
  return { 
    ...
    collectionsCreated: results.filter(r => r.status === 'created').length,
    collectionsExisting: results.filter(r => r.status === 'exists').length,
    collectionsFailed: results.filter(r => r.status === 'error').length
  };
}, { requireMaster: true });